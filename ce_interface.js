...
ce.prototype.makeInterface = function() {
  ...
  this.resultTextarea.parentNode.insertBefore(
    wrapper,
    this.resultTextarea.nextSibling
  );
...
ce.prototype.tabKeyPressed = function(event) {
  ...
  if (!toolbar[0].className.includes(this.BUTTONS_TOGGLED_CLASSNANE)) {
    ...
    toolbar[0].className = toolbar[0].className.replace(
      this.BUTTONS_TOGGLED_CLASSNANE,
      ""
    );
    this.toolbarOpened = false;
  }
...
ce.prototype.make = (function(window) {
  ...
  function node(id, content) {
    ...
    node.className += "node";
    node.innerHTML =
      '<p class="ce_node_content" contenteditable="true">' +
      (content || "") +
      "</p>";
